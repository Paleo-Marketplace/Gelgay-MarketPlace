...
export default function MasonryFeed({
  ...
  return (
    <section id="products" className="py-20 bg-[#FAF8F5] dark:bg-[#141312] border-b border-[#E8E4DC] dark:border-[#33302B]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-6 border-b border-[#E8E4DC] dark:border-[#33302B]">
          <div>
        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            return (
              <div
                key={product.id}
                className="group bg-white/85 dark:bg-[#1E1C1A]/95 backdrop-blur-md border border-white/70 dark:border-[#33302B] overflow-hidden flex flex-col hover:border-[#C85A32]/60 hover:bg-white/95 dark:hover:bg-[#252320] transition-all duration-500 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-2xl"
              >
                {/* Image Container */}
                <div className="relative aspect-4/3 bg-[#EFECE6] dark:bg-[#252320] overflow-hidden">
                  <img
                    src={product.image}
                    alt={product.title}
                  ...
                  <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
                    <span className="px-2.5 py-1 bg-[#1F1E1B]/80 dark:bg-[#141312]/90 text-[#FAF8F5] backdrop-blur-md text-[10px] font-mono tracking-wider uppercase">
                      {product.tag}
                    </span>
                    ...
                    <button
                      ...
                      className={`pointer-events-auto w-8 h-8 rounded-full flex items-center justify-center transition-colors shadow-sm ${
                        isWishlisted
                          ? 'bg-[#C85A32] text-white'
                          : 'bg-white/80 dark:bg-[#2B2824]/90 backdrop-blur-md text-[#1F1E1B] dark:text-[#FAF8F5] hover:bg-white dark:hover:bg-[#363430]'
                      }`}
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex justify-center">
                    <button
                      ...
                      className="px-4 py-2 bg-white dark:bg-[#1F1E1B] text-[#1F1E1B] dark:text-[#FAF8F5] font-mono text-xs uppercase tracking-wider flex items-center gap-1.5 hover:bg-[#C85A32] hover:text-white transition-colors"
                    >
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  <div>
                    {/* Location & Rating */}
                    <div className="flex items-center justify-between text-xs font-mono text-[#7C776E] dark:text-[#A8A296] mb-2">
                      <span className="flex items-center gap-1 text-[#524E46] dark:text-[#D1CBC0]">
                    ...
                    <h3 
                      onClick={() => onSelectProduct(product)}
                      className="font-serif text-xl font-medium text-[#1F1E1B] dark:text-[#FAF8F5] hover:text-[#C85A32] dark:hover:text-[#EB5B00] transition-colors cursor-pointer line-clamp-2 leading-snug"
                    >
                      {product.title}
                    </h3>
                  </div>
                  ...
                  <div className="pt-3 border-t border-[#E8E4DC] dark:border-[#33302B] flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-mono text-[#7C776E] dark:text-[#A8A296] uppercase tracking-wider block">
                        Condition
                      </span>
                      <span className="font-mono text-xs font-semibold text-[#1F1E1B] dark:text-[#FAF8F5]">
                        {product.condition}
                      </span>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <span className="font-serif text-lg font-bold text-[#1F1E1B] dark:text-[#FAF8F5]">
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 font-mono text-xs uppercase tracking-wider font-semibold transition-all flex items-center justify-center gap-2 ${
                      isInCart
                  ...
                  >
                    {isInCart ? (
                      <>
                        <Check className="w-4 h-4" /> In Cart / Escrow Locked
                      </>
                    ) : (
                      <>
                        <Plus className="w-4 h-4" /> Reserve in Escrow
                      </>
                    )}
                  </button>
                </div>
